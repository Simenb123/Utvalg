...
class AnalysePage(ttk.Frame):
    def __init__(self, master, controller=None, bus=None, **kwargs):
        ...
        self._pivot_sort_col = "Konto"
        self._pivot_sort_asc = True
        ...
        self._build_ui()
        self._wire_bus()
        # Juster startbredden på pivot-panelet når layouten er klar
        self.after(0, self._init_pivot_width)
        # Autoload dataset fra session etter en liten delay
        self.after(350, self._autoload_from_session)
    ...
    def _build_ui(self):
        ...
        self.cmb_dir.set("Alle")
        self.cmb_dir.pack(side="left", padx=(4, 8))
        ...
        ttk.Button(bar, text="Bruk filtre", command=lambda: self._schedule_apply(force_now=True)).pack(
            side="left", padx=(8, 0)
        )
        ...
        self.pin_menu_btn = ttk.Menubutton(bar, text="Pinned kolonner")
        self.pin_menu = tk.Menu(self.pin_menu_btn, tearoff=False)
        self.pin_menu_btn["menu"] = self.pin_menu
        self.pin_menu_btn.pack(side="right", padx=(12, 8))
        self.btn_cols = ttk.Button(bar, text="Kolonner…", command=self._open_column_chooser)
        self.btn_cols.pack(side="right")

        # Panedwindow lagres som self.body, slik at vi kan justere sash-posisjon senere
        self.body = ttk.Panedwindow(self, orient="horizontal")
        self.body.grid(row=2, column=0, sticky="nsew", padx=2, pady=6)

        # Venstre: pivot per konto
        ...
        self.pivot = ttk.Treeview(
            left,
            columns=["Konto", "Kontonavn", "Sum", "Antall"],
            show="headings",
            selectmode="extended",
            takefocus=True,
            height=22,  # flere synlige rader
        )
        ...
        # Gi venstresiden (pivot) noe mer plass utgangsmessig.
        # Vi bruker fortsatt 3:5 som vekt, men justerer sashpos eksplisitt etter at layout er klar.
        self.body.add(left, weight=3)
        self.body.add(right, weight=5)

        status = ttk.Frame(self)
        status.grid(row=3, column=0, sticky="ew", padx=6, pady=(0, 6))
        self.lbl_status = ttk.Label(status, text="Transaksjoner: 0 rader")
        self.lbl_status.pack(side="left")

        self._refresh_pinned_menu()
        self.pivot.bind("<<TreeviewSelect>>", lambda e: self._refresh_summary())

    def _init_pivot_width(self):
        """
        Justerer startposisjonen til deleren i Panedwindow slik at
        'Pivot pr. konto'-listen får en fornuftig bredde ved åpning.
        """
        pw = getattr(self, "body", None)
        if pw is None:
            return

        total = pw.winfo_width()
        if total <= 1:
            # Layout ikke helt klar enda – prøv igjen om litt
            self.after(50, self._init_pivot_width)
            return

        try:
            # Gi venstresiden ca. 40 % av bredden (justér etter smak)
            pw.sashpos(0, int(total * 0.40))
        except Exception:
            pass
    ...
    def _on_limit_changed(self, *_):
        ...
        try:
            prefs.set("analyse.display_limit", self._display_limit)
        except Exception:
            pass
    ...
    def _wire_bus(self):
        try:
            if self.bus is None or not hasattr(self.bus, "on"):
                return
    ...
    def _autoload_from_session(self):
        ...
        if isinstance(df, pd.DataFrame) and not df.empty:
            if id(df) != self._last_session_df_id or len(df) != (self._last_session_len or -1):
                self._last_session_df_id = id(df)
                self._last_session_len = len(df)
    ...
    def _try_dataset_from_session(self) -> Tuple[Optional[pd.DataFrame], str]:
        try:
            sm = importlib.import_module("session")
        except Exception:
            return (None, "")
    ...
    def _schedule_apply(self, force_now: bool = False):
        if self._apply_job_id is not None:
            try:
                self.after_cancel(self._apply_job_id)
            except Exception:
                pass
    ...
    def _build_pivot_cached(self, df: pd.DataFrame, state_key: tuple):
        if df is None or df.empty or "Beløp" not in df.columns or "Konto" not in df.columns:
            for iid in self.pivot.get_children(""):
                self.pivot.delete(iid)
            return
        ...
        if col == "Konto":
            try:
                tmp = grp.assign(
                    _k=pd.to_numeric(
                        grp["Konto"].astype(str).str.extract(r"(\d+)")[0],
                        errors="coerce",
                    )
                )
                grp = (
        ...
        for _, r in grp.iterrows():
            add(
                ...
                values=[
                    r.get("Konto", ""),
                    r.get("Kontonavn", ""),
                    format_number_no(r.get("Sum", 0.0), 2),
                    int(r.get("Antall", 0)),
                ],
            )
    ...
    def _refresh_summary(self):
        ...
        if df is None or df.empty:
            self.lbl_summary.configure(text="Oppsummering: rader=0 | sum=0,00 (viser 0)")
            return
        ...
        # Sett tekst
        txt = (
            f"Oppsummering: rader={n:,} | bilag={unique_bilag:,} | sum={sum_txt}"
