...
def build_client_store_widgets(parent: tk.Widget, *, init_client: str = "", init_year: str = "2025") -> _StoreWidgets:
    frame = ttk.LabelFrame(parent, text="Datakilde")
    ...
    btn_pick_client = ttk.Button(frame, text="Søk…", width=8)
    btn_pick_client.grid(row=0, column=2, sticky="w", padx=4, pady=4)

    btn_settings = ttk.Button(frame, text="Innstillinger…", width=14)
    ...
    ent_year = ttk.Entry(frame, textvariable=year_var, width=6)
    ent_year.grid(row=0, column=5, sticky="w", padx=6, pady=4)

    ttk.Label(frame, text="HB-versjon:").grid(row=1, column=0, sticky="w", padx=6, pady=4)
    ...
    btn_versions = ttk.Button(frame, text="Versjoner…")
    btn_versions.grid(row=1, column=4, columnspan=2, sticky="ew", padx=4, pady=4)

    lbl_storage = ttk.Label(frame, textvariable=tk.StringVar(value="Datamappe: (ukjent)"))
