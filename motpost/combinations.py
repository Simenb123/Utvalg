...
def build_motkonto_combinations(
    ...
    *,
    ...
    # Direction filter is applied only on the *selected side*.
    ...
    if dir_norm.startswith("deb"):
        ...
        dir_mask = pd.Series([True] * len(df), index=df.index)

    sel_mask = df["_konto"].isin(sel) & dir_mask
    df["_belop_valgt"] = belop.where(sel_mask, 0.0)
...
def build_bilag_to_motkonto_combo(
    ...
    *,
    ...
    for bilag_str, grp in df.groupby("Bilag_str", sort=False):
        bilag_str = _bilag_str(bilag_str)
        # NB: bilag_str kan være "" (mangler bilagsnummer). Da behandles det
        # som én egen gruppe. Det er ikke perfekt, men gjør at summer og
        # eksport kan avstemmes mot pivot.

        motkonto_set = {k for k in grp["Konto_str"].dropna().astype(str) if k.strip()} - selected_set
        combo = ", ".join(sorted(motkonto_set))
        bilag_to_combo[bilag_str] = combo if combo else empty_label

    return bilag_to_combo
