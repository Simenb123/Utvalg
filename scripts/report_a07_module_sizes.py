...
def _report(paths: list[Path], *, title: str) -> None:
    print(title)
    print(f"{'Lines':>6}  {'Budget':>6}  {'Status':<4}  Path")
    print("-" * 72)
    for path in paths:
        lines = _line_count(path)
        budget = _budget_for(path)
        status = "OK" if lines <= budget else "OVER"
        print(f"{lines:6}  {budget:6}  {status:<4}  {path.relative_to(ROOT).as_posix()}")
    print()


def main() -> None:
    _report(_iter_code_files(), title="A07 Code Modules")
    _report(_iter_test_files(), title="A07 Test Modules")
