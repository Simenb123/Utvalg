# -*- coding: utf-8 -*-
"""
dataset_pane.py – R12f+hotfix
-----------------------------
Endringer i denne hotfixen:
- Støtter kwargs `on_dataset_ready=` (callable). Denne tas UT av kwargs FØR super().__init__(),
  slik at vi ikke sender ukjente ttk-ops til ttk.Frame (fikser: TclError: unknown option "-on_dataset_ready").
- Kaller `on_dataset_ready(df)` etter vellykket bygging av datasett.
- Beholder R12f-funksjonalitet: loading overlay, ML-hukommelse (.ml_map.json), usecols for rask innlasting,
  robust norsk/internasjonal parsing og best-effort-feilhåndtering.

Avhengigheter (ingen nye): pandas, numpy, openpyxl, standardbibliotek.
"""
...
try:
    ...
    class bus:  # type: ignore
        @staticmethod
        def emit(_name, _payload=None): pass
...
class DatasetPane(ttk.Frame):
    ...
    def _build_ui(self) -> None:
        ...
        # Filrad
        rowf = ttk.Frame(self); rowf.grid(row=0, column=0, sticky="ew", padx=8, pady=(8,4))
        ...
        # Mapping
        frm = ttk.LabelFrame(self, text="Kolonnemapping"); frm.grid(row=1, column=0, sticky="ew", padx=8, pady=8)
        frm.columnconfigure(1, weight=1)
        r=0
        for canon in CANON:
            ttk.Label(frm, text=f"{canon}:").grid(row=r, column=0, sticky="w", padx=6, pady=2)
            cb = ttk.Combobox(frm, values=[], state="readonly", width=50)
            cb.grid(row=r, column=1, sticky="ew", padx=6, pady=2)
            self.combos[canon] = cb
            r += 1

        # Knapper
        rowb = ttk.Frame(self); rowb.grid(row=2, column=0, sticky="ew", padx=8, pady=(2,8))
    ...
    def _choose_file(self):
        p = filedialog.askopenfilename(title="Velg fil", filetypes=[("Regnskapsdata","*.xlsx;*.xls;*.csv;*.txt"),("Alle","*.*")])
        if not p: return
        self.path_var.set(p)
        self._load_headers()
    ...
    def _load_headers(self):
        ...
        if not p or not os.path.exists(p):
            messagebox.showwarning("Fil", "Velg gyldig fil først."); return
    ...
    def _build_dataset(self):
        ...
        # samle mapping
        mapping = {canon: self.combos[canon].get().strip() for canon in CANON if canon in self.combos and self.combos[canon].get().strip()}
