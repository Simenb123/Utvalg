...
_BASIS_VALUES = ["", "UB", "IB", "Endring", "Debet", "Kredit"]
_EXPECTED_SIGN_VALUES = ["", "-1", "0", "1"]
_SETTINGS_ROWS = (
...
def open_matcher_admin(
    ...
    *,
    ...
    ttk.Combobox(form, textvariable=basis_var, state="readonly", values=_BASIS_VALUES, width=18).grid(
        row=4,
        column=1,
        sticky="w",
        pady=(0, 4),
    ...
    ttk.Label(form, text="Nokkelord\nKomma eller Ã©n per linje").grid(row=7, column=0, sticky="nw", pady=(0, 4))
    ...
    ttk.Label(form, text="Special add\nFormat: konto | basis | weight").grid(
        row=9,
        column=0,
        sticky="nw",
        pady=(0, 4),
    ...
    ttk.Label(form, text="Aliaser\nFormat: noekkel = alias1, alias2").grid(
        row=10,
        column=0,
        sticky="nw",
        pady=(8, 4),
    ...
    settings_vars = {name: tk.StringVar(value=str(settings[name])) for name in matcher_settings_defaults}
    for row_idx, (name, label) in enumerate(_SETTINGS_ROWS):
    ...
    ttk.Label(
        ...
        wraplength=760,
        justify="left",
    ).grid(row=len(_SETTINGS_ROWS), column=0, columnspan=2, sticky="w", pady=(10, 0))
    ...
    def _available_codes() -> list[str]:
        codes = {str(code).strip() for code in _rules().keys() if str(code).strip()}
    ...
    def _refresh_code_tree(selected_code: str | None = None) -> None:
        ...
        for code in _available_codes():
            ...
            code_tree.insert("", "end", iid=code, values=(code, "Regel" if has_rule else "Ingen regel", label))
    ...
    def _delete_rule() -> None:
        ...
        if not code or code not in _rules():
            messagebox.showinfo("Matcher-admin", "Velg en regel som finnes fÃ¸rst.", parent=win)
            return
    ...
    def _save_admin(refresh_after: bool) -> None:
        ...
        try:
            ...
            status_var.set(f"Lagret global rulebook og matcher-innstillinger til {saved_rulebook.parent}.")
        except Exception as exc:
    ...
    ttk.Button(action_row, text="Lagre og oppdater A07", command=lambda: _save_admin(True)).pack(
        side="left",
        padx=(6, 0),
    )
