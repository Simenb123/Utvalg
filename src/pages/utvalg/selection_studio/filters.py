...
def filter_selectionstudio_dataframe(
    ...
    *,
    ...
    # Viktig: beløpsfilter i Utvalg/Selection Studio må slå inn på BILAGS-nivå (netto pr bilag),
    # ikke på linjenivå. Ellers vil bilag med store debet/kredit-linjer som summerer til 0
    # slippe gjennom selv om "Beløp (netto) fra" er satt > 0.
    mask_keep_amount = pd.Series([True] * len(df_dir), index=df_dir.index)

    if amount_filter_active:
        if 'Bilag' in df_dir.columns:
            bilag_key = df_dir['Bilag']
            bilag_net = bel_dir.groupby(bilag_key).sum(min_count=1)
            bilag_metric = bilag_net.abs() if use_abs else bilag_net

            keep_bilag = pd.Series([True] * len(bilag_metric), index=bilag_metric.index)
            if min_amount is not None:
                keep_bilag &= bilag_metric >= min_amount
            if max_amount is not None:
                keep_bilag &= bilag_metric <= max_amount

            keep_bilag_set = set(keep_bilag[keep_bilag].index)
            mask_keep_amount = bilag_key.isin(keep_bilag_set)
        else:
            # Fallback: dersom Bilag-kolonnen mangler, filtrer per linje.
