...
from ..backend.formatters import _fmt_pct, _safe_text
...
class _OwnerDrilldownDialog(tk.Toplevel):
    def __init__(
        ...
        *,
        ...
        self._year_text = tk.StringVar(value=self._lookup_year or "-")
    ...
    def _build_header(self) -> None:
        ...
        ttk.Label(info, textvariable=self._year_text).grid(row=1, column=1, sticky="w", pady=(2, 0))
    ...
    def _load_children(self, iid: str) -> None:
        ...
        if not orgnr or not self._lookup_year:
            ...
            return
        used_year, rows = self._load_owner_rows(orgnr)
        if used_year and used_year != self._lookup_year and meta["depth"] == 0:
            self._year_text.set(f"{used_year} (siste tilgjengelige <= {self._lookup_year})")
        if not rows:
            self._tree.insert(
                iid, "end",
                text=f"Ikke importert t.o.m. {self._lookup_year}",
                values=("", "", ""),
                tags=("missing",),
            )
            self._loaded_iids.add(iid)
    ...
    def _load_owner_rows(self, orgnr: str) -> tuple[str, list[dict]]:
        ...
        try:
            ...
            rows = []
        if rows:
            return self._lookup_year, rows

        fallback = getattr(ar_store, "list_company_owners_with_fallback", None)
        if fallback is None:
            return "", []
        try:
            used_year, fallback_rows = fallback(orgnr, self._lookup_year)
            return str(used_year or ""), list(fallback_rows or [])
        except Exception:
            log.warning(
