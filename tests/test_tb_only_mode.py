...
# ---------------------------------------------------------------------------
# 7. trial_balance_reader alias and year-detection
# ---------------------------------------------------------------------------

class TestTrialBalanceReaderAliases:
    def test_saldo_i_aar_detected_as_ub(self):
        """'Saldo i aar' should be recognized as UB."""
        from trial_balance_reader import infer_trial_balance_columns
        df = pd.DataFrame({
            "Kontonr": ["1000", "3000"],
            "Kontonavn": ["Bank", "Salg"],
            "Saldo i fjor": [100.0, -200.0],
            "Saldo i aar": [150.0, -250.0],
        })
        cols = infer_trial_balance_columns(df)
        assert cols.konto == "Kontonr"
        assert cols.ub == "Saldo i aar"
        assert cols.ib == "Saldo i fjor"

    def test_year_columns_detected(self):
        """Columns named '2025' and '2024' should be detected as UB and IB."""
        from trial_balance_reader import infer_trial_balance_columns
        df = pd.DataFrame({
            "Konto": ["1000", "3000"],
            "Kontonavn": ["Bank", "Salg"],
    ...
    def test_arets_bevegelse_detected_as_netto(self):
        """'Årets bevegelse' should be recognized as netto."""
        from trial_balance_reader import infer_trial_balance_columns
        df = pd.DataFrame({
            "Konto": ["1000"],
            "Kontonavn": ["Bank"],
        ...
        cols = infer_trial_balance_columns(df)
        assert cols.netto == "Årets bevegelse"
        assert cols.ub == "UB"

    def test_read_trial_balance_with_year_cols(self, tmp_path):
        """Full read_trial_balance with year-named columns."""
