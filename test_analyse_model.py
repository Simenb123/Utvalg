...
def test_build_pivot_by_account_basic() -> None:
    ...
    # Etter set_index("Konto") flyttes kolonnen "Konto" til index og er ikke lenger en egen kolonne.
    # Vi bruker derfor index som nøkkel direkte når vi itererer.
    rows = {int(idx): row for idx, row in pivot.set_index("Konto").iterrows()}
    assert len(rows) == 2
    # Konto 1000
    r1000 = rows[1000]
